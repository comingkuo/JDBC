...
public class PreparedStatement extends com.mysql.jdbc.StatementImpl implements
		...
		public ParseInfo(String sql, MySQLConnection conn,
			...
			try {
				...
				for (i = 0; i < this.staticSql.length; i++) {
					...
					//modify it and make it store string
   ...
   public String getDateTime(String pattern){
	...
	protected ResultSetInternalMethods executeInternal(int maxRowsToRetrieve,
			Buffer sendPacket, boolean createStreamingResultSet/* in executeUpdate is false */,
			boolean queryIsSelectOnly/* in executeUpdate is false */, Field[] metadataFromCache,
			boolean isBatch)
			throws SQLException {
		try {
	 ...
	 *             if a database error occurs
	 */
	protected int executeUpdate(byte[][] batchedParameterStrings/* this is "?" in sql statement */,
			InputStream[] batchedParameterStreams/* using in BLOB */, boolean[] batchedIsStream,
	...
	protected Buffer fillSendPacket(byte[][] batchedParameterStrings,
		...
		sendPacket.setPosition(2); //reserving for storing packet length
		...
		if (ensurePacketSize != 0) {
			sendPacket.ensureCapacity(ensurePacketSize+3);
		}
		...
		for (int i = 0; i < batchedParameterStrings.length; i++) {
			...
			sendPacket.writeString(new String(this.staticSqlStrings[i]));
			//sendPacket.writeBytesNoNull(this.staticSqlStrings[i]);

			//FIXME : carefully send BLOB
			if (batchedIsStream[i]/* using in streamdata */) {
				streamToBytes(sendPacket, batchedParameterStreams[i], true,
						batchedStreamLengths[i], useStreamLengths);
			} else {
				sendPacket.writeString(new String(batchedParameterStrings[i]));
				//sendPacket.writeBytesNoNull(batchedParameterStrings[i]);
			}
		}

		sendPacket.writeString(new String(this.staticSqlStrings[batchedParameterStrings.length]));

		


		sendPacket.writePacketLen();//write packet length in packet header
	...
	protected void setBytes(int parameterIndex, byte[] x,
		...
		if (x == null) {
			...
			for (int i = 0; i < numBytes; ++i) {
				...
				/*try {
					bOut.write( unicodeEscaped((char) b).getBytes() );
				} catch (IOException e) {
					e.printStackTrace();
				}*/
				
				
				//FIXME: Didn't care about '\0'
				bOut.write(b);
				/*switch (b) {
	...
	protected void setBytesNoEscape(int parameterIndex, byte[] parameterAsBytes)
		...
		byte[] parameterWithQuotes = new byte[parameterAsBytes.length];
		...
		if (x == null) {
			...
			if (this.connection.isNoBackslashEscapesSet()) {
				...
				if (!needsHexEscape) {
					...
					StringBuffer quotedString = new StringBuffer(x.length());
					//quotedString.append('\'');
	...
	private final void streamToBytes(Buffer packet, InputStream in,
		...
		try {
			...
			while (bc > 0) {
				...
				//FIXME: using in string which have '\0'
				/*for(int i=0; i< streamConvertBuf.length; i++)
					packet.writeString( unicodeEscaped((char) streamConvertBuf[i]) );*/
				//}
