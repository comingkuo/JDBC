...
public class MysqlIO {
    ...
    public MysqlIO(String host, int port, Properties props,
        ...
        try {
	        ...
	        this.profileSql = this.connection.getProfileSql();//default false
    ...
    protected ResultSetImpl getResultSet(StatementImpl callingStatement,
        ...
        Field[] fields = null;//column result sets

        // Read in the column information

        //in stmt.execute(sql) is null
        ...
        //in stmt.execute(sql), it is !(false).
        if (!streamResults) {
            rowData = readSingleRowSet(columnCount, maxRows,
                    resultSetConcurrency, isBinaryEncoded/* false */,
    ...
    protected final Buffer readPacket() throws SQLException {
        try {
            ...
            int lengthRead = readFully(this.mysqlInput,//MysqlIODef.HEADER_LENGTH
    ...
    protected final Field unpackField(Buffer packet,
        ...
        //because if your length very long, then you will use readLong() instead readInt().
     ...
     * @throws SQLException DOCUMENT ME!
     * @throws CommunicationsException DOCUMENT ME!
     * send: user, password, database messages to handshake (modify from MySQL connector/j)
     */
    void doHandshake(String user, String password, String database)
        ...
        String encoding = jvmPlatformCharset;//client's encoding

        if (encoding == null) {
            encoding = System.getProperty("file.encoding"); //$NON-NLS-1$
        }
        
        int pktLen = encoding.length() + user.length() + password.length() + database.length() + 8;
        
        Buffer logInfo = new Buffer(pktLen);
    
        logInfo.writeInt(encoding.length());
        logInfo.writeString(encoding);
        
        logInfo.writeInt(user.length());
        logInfo.writeString(user);
    
        //TODO : socket SSL for password
        logInfo.writeInt(password.length());
        logInfo.writeString(password);
    
        logInfo.writeInt(database.length());
        logInfo.writeString(database);
    ...
    */
    final ResultSetRow nextRow(Field[] fields, int columnCount,
        ...
        boolean useBufferRowIfPossible/*false*/,
        boolean useBufferRowExplicit,
        boolean canReuseRowPacketForBufferRow/*false*/, Buffer existingRowPacket/*null*/)
        throws SQLException {
        ...
        if (existingRowPacket == null) {
        	rowPacket = checkErrorPacket();//reuseAndReadPacket()
...
//
    ...
    final Buffer sendCommand(int command, String extraData/* in sqlQueryDirect is null */, Buffer queryPacket,
        boolean skipCheck/* sqlQueryDirct false*/, String extraDataCharEncoding/* null */, int timeoutMillis/* 0 */)
        throws SQLException {
        ...
        //
        this.enablePacketDebug = this.connection.getEnablePacketDebug(); // default is false
        this.readPacketSequence = 0;
        ...
        if (timeoutMillis != 0) {
        	try {
        		oldTimeout = this.mysqlConnection.getSoTimeout();//socket timeout
        ...
        try {
            ...
            checkForOutstandingStreamingData();//�ˬdthis.streamdata ���O�_��content, ��? flush it
            ...
            Buffer returnPacket = new Buffer(1024);
            ...
            if (!skipCheck) {
        		...
        		this.mysqlInput.read(b, 0, 3);
        		returnPacket.writeBytesNoNull(b);
        		returnPacket.setPosition(1);
            }
    ...
    throws Exception {
    	//default is 0, counting numbers of statements
    	...
    	try {
	    	...
	    	if (query != null) {
	    		...
	    		if (this.sendPacket == null) {
	    			this.sendPacket = new Buffer(query.getBytes().length + 3);//two for length, one for command
	    		} else {
	    			this.sendPacket.clear();
	    		}
    ...
    protected final ResultSetImpl readResultsForQueryOrUpdate(
        ...
        //TODO :not read yet
            ...
            // changed once we've connected, default is true.
    ...
    private com.mysql.jdbc.ResultSetImpl buildResultSetWithRows(
        ...
        //default is this case in stmt.execsql(sql)
    ...
    private com.mysql.jdbc.ResultSetImpl buildResultSetWithUpdates(
        ...
        try {
        	// should we use the new larger update counts? , default is false
