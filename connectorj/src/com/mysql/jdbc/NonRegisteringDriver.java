...
public class NonRegisteringDriver implements java.sql.Driver {
	...
	private static final String SQLMR_PREFIX = "jdbc:sqlmr://";
	...
	public java.sql.Connection connect(String url, Properties info)
		...
		if (!"1".equals(props.getProperty(NUM_HOSTS_PROPERTY_KEY))) {
			//TODO: (kuo) need to fix(not trace yet)
	...
	public DriverPropertyInfo[] getPropertyInfo(String url, Properties info)
		...
		if ((url != null) && url.startsWith(SQLMR_PREFIX)) { //$NON-NLS-1$
			info = parseURL(url, info);
		}
		...
		DriverPropertyInfo portProp = new DriverPropertyInfo(PORT_PROPERTY_KEY, //$NON-NLS-1$
				info.getProperty(PORT_PROPERTY_KEY, "8765")); //$NON-NLS-1$ //$NON-NLS-2$
...
//TODO :(kuo) add jdbc:sqlmr
		if (!StringUtils.startsWithIgnoreCase(url, URL_PREFIX)
				...
				&& !StringUtils.startsWithIgnoreCase(url,
						REPLICATION_URL_PREFIX)
			    && !StringUtils.startsWithIgnoreCase(url, SQLMR_PREFIX)) { //$NON-NLS-1$
		...
		if ((hostStuff != null) && (hostStuff.trim().length() > 0)) {
			...
			for (String hostAndPort : hosts) {
				...
				if (hostPortPair[PORT_NUMBER_INDEX] != null) {
					...
					urlProps.setProperty(PORT_PROPERTY_KEY + "." + numHosts, "8765");
				}
			}
		} else {
			numHosts = 1;
			urlProps.setProperty(HOST_PROPERTY_KEY + ".1", "localhost");
			urlProps.setProperty(PORT_PROPERTY_KEY + ".1", "8765");
		}
	...
	public int port(Properties props) {
		return Integer.parseInt(props.getProperty(PORT_PROPERTY_KEY, "8765")); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	/**
	 * Expands hosts of the form address=(protocol=tcp)(host=localhost)(port=8765)
