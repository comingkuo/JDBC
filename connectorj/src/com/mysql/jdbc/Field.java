...
public class Field {
	...
	Field(MySQLConnection conn, byte[] buffer, int databaseNameStart,
		...
		//TODO :(kuo) may need to fix
		this.sqlType = MysqlDefs.mysqlToJavaType(this.mysqlType);//mysqlType default is -1

        checkForImplicitTemporaryTable();//if tableName prefix = "#sql_", this.isImplicitTempTable is true 
		// Re-map to 'real' blob type, if we're a BLOB
        boolean isFromFunction = this.originalTableNameLength == 0;//false 
        
        
        //TODO :(kuo) this.sqlType,this.mysqltype
