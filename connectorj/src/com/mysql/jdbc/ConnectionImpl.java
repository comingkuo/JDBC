...
public class ConnectionImpl extends ConnectionPropertiesImpl implements
   ...
   public MySQLConnection getLoadBalanceSafeProxy() {
	...
	/** The port number we're connected to (defaults to 8765) */
	...
	private void coreConnect(Properties mergedProps) throws SQLException,
		...
		if (protocol != null) {
			...
			if ("tcp".equalsIgnoreCase(protocol)) {
				...
				newPort = parsePortNumber(mergedProps.getProperty(NonRegisteringDriver.PORT_PROPERTY_KEY, "8765"));
	...
	//In preparestatement, the buffer packet could not null.
	public ResultSetInternalMethods execSQL(StatementImpl callingStatement, String sql, int maxRows,
			Buffer packet, int resultSetType, int resultSetConcurrency,
			boolean streamResults, String catalog,
		...
		synchronized (this.mutex) {
			...
			/* in stmt.execute(sql) is null, using in catch error msgs, but in preparedStatement is not null*/
			if (packet != null) {
			...
			if ((getHighAvailability()/* default is false */)
					&& (this.autoCommit || getAutoReconnectForPools())
					&& this.needsPing && !isBatch) {
				try {
					//TODO : same as StatementImpl.java line697 doPingInstead();
					pingInternal(false, 0);
		...
		if (versionMeetsMinimum(5, 0, 0)) {
			...
			if (!getCacheCallableStatements()) {
				...
				synchronized (this.parsedCallableStatementCache) {
					CompoundCacheKey key = new CompoundCacheKey(getCatalog(), sql); //use to compare 2 preparedcall whether it is equal or not.
